...
class RehabilitationList extends Component<Props, object> {
  ...
  componentDidMount() {
    ...
    this.props.navigation.addListener('willFocus', (event: any) => {
      if (event.state != undefined && event.state.routeName == 'RehabilitationList') {
  ...
  private renderExercise = (data: any) => {
    return (
      <RehabilitationCell isHome={false} data={data} onPress={() => this.props.navigation.navigate('WorkoutDetails', { id: data.item.id })} />
    )
  }
          ...
          </View> */}
        {
          this.state.workoutsArr.length == 0 ?
            <View style={{ flex: 1, justifyContent: 'center' }}>
              <Text style={styles.noWorkoutTxt}>{'No data found.'}</Text>
            </View> :
            <FlatList
...
export default connect(mapStateToProps, mapDispatchToProps)(RehabilitationList);